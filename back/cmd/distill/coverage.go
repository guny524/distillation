...
func coverageCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			// Idempotent DDL migrates a pre-M3 table so the source_lane/artifact
			// coverage queries below see the new columns.
			if err := loader.CreateTable(ctx, conn); err != nil {
				fmt.Fprintf(os.Stderr, "[coverage] create table failed: %v\n", err)
				return err
			}
