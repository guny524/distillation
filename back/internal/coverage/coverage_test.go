...
func TestBuildEmptyCoverage_AllKeysPresent(t *testing.T) {
	...
	// All axis maps (4-axis + M3 lane/source) must be present.
	assert.NotNil(t, cov)
	...
	assert.IsType(t, map[string]int{}, cov.SourceLane)
	assert.IsType(t, map[string]int{}, cov.ArtifactSource)
	assert.IsType(t, map[string]int{}, cov.LaneXSource)
...
func TestBuildEmptyCoverage_TotalCountZero(t *testing.T) {
	...
	assert.Empty(t, cov.SourceLane)
	assert.Empty(t, cov.ArtifactSource)
	assert.Empty(t, cov.LaneXSource)
}

func TestQuerySourceLaneCounts(t *testing.T) {
	q := &dbtest.MockQuerier{
		QueryFn: func(_ context.Context, sql string, _ ...any) (pgx.Rows, error) {
			assert.Contains(t, sql, "source_lane")
			assert.Contains(t, sql, "IS NOT NULL")
			return dbtest.NewMockRows([]string{"source_lane", "count"}, [][]any{
				{"ko", 12}, {"en", 8},
			}), nil
		},
	}

	counts, err := QuerySourceLaneCounts(context.Background(), q)

	require.NoError(t, err)
	assert.Equal(t, map[string]int{"ko": 12, "en": 8}, counts)
}

func TestQueryArtifactSourceCounts(t *testing.T) {
	q := &dbtest.MockQuerier{
		QueryFn: func(_ context.Context, sql string, _ ...any) (pgx.Rows, error) {
			assert.Contains(t, sql, "jsonb_array_elements(artifact_refs)")
			assert.Contains(t, sql, "source_type")
			return dbtest.NewMockRows([]string{"source_type", "count"}, [][]any{
				{"arxiv", 5}, {"k8s", 3},
			}), nil
		},
	}

	counts, err := QueryArtifactSourceCounts(context.Background(), q)

	require.NoError(t, err)
	assert.Equal(t, map[string]int{"arxiv": 5, "k8s": 3}, counts)
}

func TestQueryLaneSourceCross_KeyFormat(t *testing.T) {
	q := &dbtest.MockQuerier{
		QueryFn: func(_ context.Context, sql string, _ ...any) (pgx.Rows, error) {
			assert.Contains(t, sql, "source_lane")
			assert.Contains(t, sql, "jsonb_array_elements(artifact_refs)")
			return dbtest.NewMockRows([]string{"source_lane", "source_type", "count"}, [][]any{
				{"ko", "arxiv", 4}, {"en", "k8s", 2},
			}), nil
		},
	}

	counts, err := QueryLaneSourceCross(context.Background(), q)

	require.NoError(t, err)
	assert.Equal(t, map[string]int{"ko:arxiv": 4, "en:k8s": 2}, counts)
}

func TestQueryArtifactSourceCounts_QueryError(t *testing.T) {
	q := &dbtest.MockQuerier{
		QueryFn: func(_ context.Context, _ string, _ ...any) (pgx.Rows, error) {
			return nil, fmt.Errorf("jsonb query failed")
		},
	}

	_, err := QueryArtifactSourceCounts(context.Background(), q)

	require.Error(t, err)
	assert.Contains(t, err.Error(), "query_artifact_source_counts")
}
...
func TestBuildCoverage_AssemblesAllAxes(t *testing.T) {
	...
	q := &dbtest.MockQuerier{
		...
		QueryFn: func(_ context.Context, sql string, _ ...any) (pgx.Rows, error) {
			...
			case strings.Contains(sql, "source_lane") && strings.Contains(sql, "jsonb_array_elements"):
				// lane x source cross
				return dbtest.NewMockRows([]string{"source_lane", "source_type", "count"}, [][]any{
					{"ko", "arxiv", 9},
				}), nil
			case strings.Contains(sql, "jsonb_array_elements"):
				// artifact source counts
				return dbtest.NewMockRows([]string{"source_type", "count"}, [][]any{
					{"arxiv", 11},
				}), nil
			case strings.Contains(sql, "source_lane"):
				// source lane counts
				return dbtest.NewMockRows([]string{"source_lane", "count"}, [][]any{
					{"ko", 15},
				}), nil
	...
	assert.Equal(t, map[string]int{"ko": 15}, cov.SourceLane)
	assert.Equal(t, map[string]int{"arxiv": 11}, cov.ArtifactSource)
	assert.Equal(t, map[string]int{"ko:arxiv": 9}, cov.LaneXSource)
